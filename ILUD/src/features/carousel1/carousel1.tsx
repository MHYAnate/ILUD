...
const Carousel: React.FC<CarouselProps> =({items}) => {
  ...
  const handleNext = () => {
    setActiveIndex((prevIndex) => (prevIndex === random - 1 ? 0 : prevIndex + 1));
  };

  const handlePrev = () => {
    setActiveIndex((prevIndex) => (prevIndex === 0 ? random - 1 : prevIndex - 1));
  };
        ...
        <div className={styles.imgform}>
          ...
          <div className={styles.form}>
            <div className={styles.headsvg} > 
              <svg xmlns="http://www.w3.org/2000/svg" fill="gold" viewBox="3 0 24 24" strokeWidth={1.5} stroke="gold" className={styles.svg}>
              ...
              <button className={styles.btn}>
                <span className={styles.btnSpan1}>Like</span> {items[activeIndex].like}
              </button>
              <button className={styles.btn}> {items[activeIndex].dislike}
